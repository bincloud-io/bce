...
public final class ValidationState {
  ...
  private Map<ValidationGroup, Collection<ErrorMessage>> createDerivedGroups(
      ValidationGroup baseGroup, Map<ValidationGroup, Collection<ErrorMessage>> errorMessages) {
  ...
  private Map<ValidationGroup, Collection<ErrorMessage>> getGroupedErrors() {
    return errorMessages.entrySet().stream()
        .filter(entry -> !ValidationGroup.UNGROUPED.equals(entry.getKey()))
  ...
  private Collection<ErrorMessage> getUnmodifiableMessagesCollection(
  ...
  private static class DefaultErrorState implements ErrorState {
    ...
    public Collection<GroupedError> getGroupedErrors() {
      return Collections.unmodifiableCollection(groupedErrors.entrySet().stream()
          .map(DefaultGroupedError::new).collect(Collectors.toSet()));
    }
  }
