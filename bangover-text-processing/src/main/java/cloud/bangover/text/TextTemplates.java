...
public class TextTemplates {
  ...
  public static final DefaultTextTemplate createBy(Object object) {
    return new DefaultTextTemplate(Optional.ofNullable(object).map(Object::toString),
        new HashMap<>());
  }
  ...
  /**
   * Wrap an already existing text message. The text template is created and initialized by the
   * source template text and parameters. For example you have an unknown type text template and you
   * need to reconfigure the source message (for example replace parameters value). You can wrap the
   * source template and configure it using {@link DefaultTextTemplate#withNullPattern(String)},
   * {@link DefaultTextTemplate#withParameter(String, Object)} and other. Also it may help in the
  ...
  /**
   * This class implements the text message template, which the platform provides by default. You
  ...
  public static class DefaultTextTemplate implements TextTemplate {
    ...
    private DefaultTextTemplate(Optional<String> templateText, String nullPattern,
        Map<String, Object> parameters) {
      super();
