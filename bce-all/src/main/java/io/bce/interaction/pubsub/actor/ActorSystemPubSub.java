...
public class ActorSystemPubSub<T> implements PubSub<T> {
  ...
  private ActorSystemPubSub(@NonNull ActorSystem actorSystem,
      @NonNull ActorAddress coordinatorAddress) {
    super();
  ...
  public static Factory factory(@NonNull ActorSystem actorSystem) {
    return new Factory() {
      ...
      public <T> PubSub<T> createPubSub(@NonNull ActorName coordinatorName) {
        ActorAddress coordinatorAddress =
            actorSystem.actorOf(coordinatorName, MessagingCoordinatorActor.factory());
  ...
  private class ActorSystemSubscribtion implements Subscribtion {
    ...
    public void unsubscribe() {
      actorSystem
          .tell(Message.createFor(coordinatorAddress, new Unsubscribe(topic, subscriberAddress)));
    }
  }
  ...
  private class SubscriberActor<M> extends Actor<M> {
    ...
    public SubscriberActor(@NonNull Context context, @NonNull Class<M> messageType,
        @NonNull Topic topic, @NonNull Subscriber<M> subscriber) {
      super(context);
